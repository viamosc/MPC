...
(()=>{
    const e = new Error("Cannot find module '@/lib/requests'");
...
function DashboardPage() {
    ...
    function handleCancelRequest(player) {
        setMyRequest(null);
        cancelPresenceRequest(player.id).catch((err)=>setPlayersError(err.message));
    }
    function handleApproveRequest(request) {
        setRequests((prev)=>prev.filter((r)=>r.id !== request.id));
        resolvePresenceRequest(request.id).catch((err)=>setPlayersError(err.message));
    ...
    function handleDenyRequest(requestId) {
        ...
        resolvePresenceRequest(requestId).catch((err)=>setPlayersError(err.message));
    }
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "DashboardPage.useEffect": ()=>{
            ...
            if (isAdmin) {
                let cancelled = false;
                const refresh = {
                    "DashboardPage.useEffect.refresh": ()=>getAllPresenceRequests().then({
                ...
                refresh();
                const unsubscribe = subscribeToAllPresenceRequests(refresh);
                return ({
                    "DashboardPage.useEffect": ()=>{
                        cancelled = true;
                        unsubscribe();
            ...
            if (currentPlayerId) {
                let cancelled = false;
                const refresh = {
                    "DashboardPage.useEffect.refresh": ()=>getOwnPresenceRequest(currentPlayerId).then({
                            "DashboardPage.useEffect.refresh": (data)=>{
                ...
                refresh();
                const unsubscribe = subscribeToOwnPresenceRequest(currentPlayerId, refresh);
                return ({
                    "DashboardPage.useEffect": ()=>{
                        cancelled = true;
                        unsubscribe();
...
//# sourceMappingURL=_20qr238._.js.map