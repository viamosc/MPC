...
export async function getAppState() {
  const { data, error } = await supabase
    .from("app_state")
    .select("courts, queues, duration, announcement, auto_duration")
    .eq("id", STATE_ROW_ID)
    .single();
...
export async function saveAutoDuration(autoDuration) {
  const { error } = await supabase
    .from("app_state")
    .update({ auto_duration: autoDuration, updated_at: new Date().toISOString() })
    .eq("id", STATE_ROW_ID);
  if (error) throw new Error(error.message);
}

export async function saveDurationManual(duration) {
  const { error } = await supabase
    .from("app_state")
    .update({ duration, auto_duration: false, updated_at: new Date().toISOString() })
    .eq("id", STATE_ROW_ID);
